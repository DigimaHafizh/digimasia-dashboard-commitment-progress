...
> **v2.1 revision summary:** active commitments reset to empty each cycle (history
> preserved); users submit via "Add my Commitment"; Admin direct editing removed in favor
> of an Accept/Reject review workflow with a mandatory general comment; the `NOT_MEASURABLE`
> / `TOO_OPTIMISTIC` / `NEW_USER` enum flags and the `Not Started` status were removed;
> review comments appear in the user's own form, not as dashboard tooltips.
>
> Note: this PRD predates the app's current implementation. See the main
> [README.md](./README.md) for what's actually shipped today — most notably, the public
> dashboard described below (§3.1, §4.1) has since been removed, and progress updates
> (not just the initial commitment) now also go through Admin review.
...
4. **[UPDATED v2.1] Submit / Add Commitment:** 
...
7. **[UPDATED v2.0] History & Activity Logging:** The system inserts a new entry into the progress history table **without overwriting or deleting any past updates or historical cycle data**. The main dashboard updates immediately. The user can view their complete personal timeline of changes (status updates, added challenges, impacts, or review feedback) directly under their form workspace.

### 4.2 Superuser Flow (Admin)
1. **Admin Login:** The Admin authenticates using a dedicated Admin PIN.
2. **[UPDATED v2.0] Dashboard Review (`On Review` Filter):** The Admin toggles the `On Review` filter to isolate newly submitted or resubmitted user commitments awaiting evaluation.
